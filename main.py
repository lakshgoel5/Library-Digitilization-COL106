...
def check_lib(lib, unique_words, word_to_books, sort_output=True):
    ...
    if lib.count_distinct_words("booka") == len(unique_words[0]) and lib.count_distinct_words("booke") == len(unique_words[1]):
...
def main():
    book_titles = ["booka", "booke"]
    texts = [["The", "name", "of", "this", "book", "contains", "a", "number", "of", "words", "that", "are","The"],
    ...
    for book, text in zip(book_titles, texts):
        for word in text:  # Corrected line
